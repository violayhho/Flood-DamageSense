...
class VSSM(nn.Module):
    ...
    @staticmethod
    def _make_patch_embed(in_chans=4, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
    ...
    @staticmethod
    def _make_patch_embed_v2(in_chans=4, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        assert patch_size == 4
        return nn.Sequential(
    ...
    def flops(self, shape=(4, 224, 224)):
