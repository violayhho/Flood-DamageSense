...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=4, num_classes=2, embed_dims=[64, 128, 256, 512],
...
class ChangeFormerV1BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map=3, decoder_softmax=False):
        super(ChangeFormerV1BDA, self).__init__()

        self.Tenc               = Tenc()
        
        self.convproj_damage           = convprojection_base()
        self.convproj_map              = convprojection_base()

        self.change_probability_damage = ConvLayer(8, output_damage, kernel_size=3, stride=1, padding=1)
        self.change_probability_map    = ConvLayer(8, output_map, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x1, x2):
        ...
        cp_damage = self.convproj_damage(DI)
        cp_map = self.convproj_map(DI)

        cp_damage = self.change_probability_damage(cp_damage)
        cp_map = self.change_probability_map(cp_map)

        if self.output_softmax:
            cp_damage = self.active(cp_damage)
            cp_map = self.active(cp_map)

        return cp_damage, cp_map
...
class ChangeFormerV2BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map = 3, decoder_softmax=False):
        super(ChangeFormerV2BDA, self).__init__()
        #Transformer Encoder
        self.Tenc   = Tenc()
        
        #Transformer Decoder
        self.TDec_damage   = TDec( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                            in_channels = [64, 128, 320, 512], embedding_dim= 32, output_nc=output_damage, 
                            decoder_softmax = decoder_softmax, feature_strides=[4, 8, 16, 32])
        self.TDec_map      = TDec( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
                            in_channels = [64, 128, 320, 512], embedding_dim= 32, output_nc=output_map, 
                            decoder_softmax = decoder_softmax, feature_strides=[4, 8, 16, 32])
        #Final activation
    ...
    def forward(self, x1, x2):
        ...
        cp_damage = self.TDec_damage(DI)
        cp_map = self.TDec_map(DI)

        if self.decoder_softmax:
            cp_damage = self.output_activation(cp_damage)
            cp_map = self.output_activation(cp_map)

        return cp_damage, cp_map

# ChangeFormerV3:
# Feature differencing and pass it through Transformer decoder
class ChangeFormerV3BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map = 3, decoder_softmax=False):
        super(ChangeFormerV3BDA, self).__init__()
        #Transformer Encoder
        ...
        #Transformer Decoder
        self.TDec_damage   = TDecV2( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                            in_channels = [64, 128, 320, 512], embedding_dim= 64, output_nc=output_damage, 
                            decoder_softmax = decoder_softmax, feature_strides=[4, 8, 16, 32])
        self.TDec_map      = TDecV2( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
                            in_channels = [64, 128, 320, 512], embedding_dim= 64, output_nc=output_map,
                            decoder_softmax = decoder_softmax, feature_strides=[4, 8, 16, 32])
    ...
    def forward(self, x1, x2):
        ...
        cp_damage = self.TDec_damage(fx1, fx2)
        cp_map = self.TDec_map(fx1, fx2)

        return cp_damage, cp_map

#Transormer Ecoder with x2, x4, x8, x16 scales
class EncoderTransformer_x2(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=4, num_classes=2, embed_dims=[32, 64, 128, 256, 512],
...
# ChangeFormerV4:
class ChangeFormerV4BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map = 3, decoder_softmax=False):
        super(ChangeFormerV4BDA, self).__init__()
        #Transformer Encoder
        self.embed_dims = [32, 64, 128, 320, 512]
        self.depths     = [3, 3, 4, 12, 3] #[3, 3, 6, 18, 3]
        self.embedding_dim = 256

        self.Tenc_x2_damage    = EncoderTransformer_x2(img_size=256, patch_size=3, in_chans=input_nc, num_classes=output_damage, embed_dims=self.embed_dims,
                 num_heads=[2, 2, 4, 8, 16], mlp_ratios=[2, 2, 2, 2, 2], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=self.depths, sr_ratios=[8, 4, 2, 1, 1])
        self.Tenc_x2_map       = EncoderTransformer_x2(img_size=256, patch_size=3, in_chans=input_nc, num_classes=output_map, embed_dims=self.embed_dims,
                num_heads=[2, 2, 4, 8, 16], mlp_ratios=[2, 2, 2, 2, 2], qkv_bias=False, qk_scale=None, drop_rate=0.,
                attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                depths=self.depths, sr_ratios=[8, 4, 2, 1, 1])
        
        #Transformer Decoder
        self.TDec_x2_damage   = DecoderTransformer_x2(input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True, 
                    in_channels = self.embed_dims, embedding_dim= 256, output_nc=output_damage, 
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16, 32])
        self.TDec_x2_map      = DecoderTransformer_x2(input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True,
                    in_channels = self.embed_dims, embedding_dim= 256, output_nc=output_map,
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16, 32])

    def forward(self, x1, x2):

        [fx1_damage, fx2_damage] = [self.Tenc_x2_damage(x1), self.Tenc_x2_damage(x2)]
        [fx1_map, fx2_map]       = [self.Tenc_x2_map(x1), self.Tenc_x2_map(x2)]

        cp_damage = self.TDec_x2_damage(fx1_damage, fx2_damage)
        cp_map = self.TDec_x2_map(fx1_map, fx2_map)

        # # Save to mat
        # save_to_mat(x1, x2, fx1, fx2, cp, "ChangeFormerV4")

        # exit()
        return cp_damage[5], cp_map[5]


#Transormer Ecoder with x2, x4, x8, x16 scales
class EncoderTransformer_v3(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=4, num_classes=2, embed_dims=[32, 64, 128, 256],
...
# ChangeFormerV5:
class ChangeFormerV5BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map = 3, decoder_softmax=False, embed_dim=256):
        super(ChangeFormerV5BDA, self).__init__()
        #Transformer Encoder
        ...
        self.Tenc_x2_damage    = EncoderTransformer_v3(img_size=256, patch_size = 3, in_chans=input_nc, num_classes=output_damage, embed_dims=self.embed_dims,
        ...
        self.Tenc_x2_map       = EncoderTransformer_v3(img_size=256, patch_size = 3, in_chans=input_nc, num_classes=output_map, embed_dims=self.embed_dims,
                num_heads = [1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
                attn_drop_rate = self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                depths=self.depths, sr_ratios=[8, 4, 2, 1])
        
        #Transformer Decoder
        self.TDec_x2_damage   = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False, 
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_damage, 
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16])
        self.TDec_x2_map      = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False,
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_map,
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16])
    ...
    def forward(self, x1, x2):
        ...
        # exit()
        return cp_damage[4], cp_map[4]

# ChangeFormerV6:
class ChangeFormerV6BDA(nn.Module):

    def __init__(self, input_nc=4, output_damage=4, output_map = 3, decoder_softmax=False, embed_dim=256):
        super(ChangeFormerV6BDA, self).__init__()
        #Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        ...
        self.Tenc_x2_damage    = EncoderTransformer_v3(img_size=256, patch_size = 7, in_chans=input_nc, num_classes=output_damage, embed_dims=self.embed_dims,
                 num_heads = [1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
                 attn_drop_rate = self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=self.depths, sr_ratios=[8, 4, 2, 1])
        self.Tenc_x2_map       = EncoderTransformer_v3(img_size=256, patch_size = 7, in_chans=input_nc, num_classes=output_map, embed_dims=self.embed_dims,
                num_heads = [1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
                attn_drop_rate = self.attn_drop, drop_path_rate=self.drop_path_rate, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                depths=self.depths, sr_ratios=[8, 4, 2, 1])
